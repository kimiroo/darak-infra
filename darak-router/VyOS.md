...
## VLANs
| VLAN | Description | IPv4 Subnet  | IPv6 Subnet            |
| ---- | ----------- | ------------ | ---------------------- |
| 5    | Network     | 10.5.0.0/16  | fdab:d9c3:fb50:5::/64  |
| 7    | VPN         | 10.7.0.0/16  | fdab:d9c3:fb50:7::/64  |
| 10   | Infra       | 10.10.0.0/16 | fdab:d9c3:fb50:10::/64 |
| 20   | Admin       | 10.20.0.0/16 | fdab:d9c3:fb50:20::/64 |
| 30   | Family      | 10.30.0.0/16 | fdab:d9c3:fb50:30::/64 |
| 70   | Trusted     | 10.70.0.0/16 | fdab:d9c3:fb50:70::/64 |
| 80   | IoT         | 10.80.0.0/16 | fdab:d9c3:fb50:80::/64 |
| 90   | Guest       | 10.90.0.0/16 | fdab:d9c3:fb50:90::/64 |
| 99   | Limbo       | 10.99.0.0/16 | fdab:d9c3:fb50:99::/64 |
...
set interface ethernet eth0 dhcpv6-options duid '00:03:00:01:18:c5:01:8b:40:f5'

# DHCPv6 Prefix Delegation (IA_PD) (DOESN'T WORK IN KOREAN ISPS)
...
#set service dhcp-server shared-network-name VLAN80 subnet 10.80.0.0/16 static-mapping home-ac ip-address 'fdab:d9c3:fb50:80:100::1'
#set service dhcp-server shared-network-name VLAN80 subnet 10.80.0.0/16 static-mapping home-ac duid '50:fd:d5:c8:81:a8'
...
## NextDNS CLI Container
```bash
configure

# Add container image
run add container image 'docker.io/nextdns/nextdns:latest'

# Create container
set container name nextdns image 'docker.io/nextdns/nextdns:latest'
set container name nextdns restart 'always'
set container name nextdns allow-host-networks
set container name nextdns arguments "run \
  -listen :53 \
  -forwarder internal.darak.dev=127.0.0.1:5353 \
  -forwarder 10.in-addr.arpa=127.0.0.1:5353 \
  -profile 10.5.0.0/16=ad1b36 \
  -profile fdab:d9c3:fb50:5::/64=ad1b36 \
  -profile 10.7.0.0/16=122774 \
  -profile fdab:d9c3:fb50:7::/64=122774 \
  -profile 10.10.0.0/16=ad1b36 \
  -profile fdab:d9c3:fb50:10::/64=ad1b36 \
  -profile 10.20.0.0/16=122774 \
  -profile fdab:d9c3:fb50:20::/64=122774 \
  -profile 10.30.0.0/16=bd8962 \
  -profile fdab:d9c3:fb50:30::/64=bd8962 \
  -profile 10.70.0.0/16=ef3c69 \
  -profile fdab:d9c3:fb50:70::/64=ef3c69 \
  -profile 10.80.0.0/16=59cb3b \
  -profile fdab:d9c3:fb50:80::/64=59cb3b \
  -profile 10.90.0.0/16=ef3c69 \
  -profile fdab:d9c3:fb50:90::/64=ef3c69 \
  -report-client-info"

# Create update scheduler
cat << 'EOF' > /config/scripts/update-nextdns.sh
#!/bin/vbash

source /opt/vyatta/etc/functions/script-template

run update container image nextdns
run restart container nextdns
EOF

chmod +x /config/scripts/update-nextdns.sh

set system task-scheduler task update_nextdns crontab-spec '30 3 * * *'
set system task-scheduler task update_nextdns executable path '/config/scripts/update-nextdns.sh'

commit comment 'container: Create NextDNS CLI container and update scheduler'
save
```
...
commit comment 'wireguard: Configure WireGuard'
save
```
